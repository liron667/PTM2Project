...
public class Model extends Observable implements Observer {
    ...
    public void GetPlane(double xStart,double yStart, double offset){
    	String dumpPos = "dump /position";
    	String splitSymbol = "[<>]";
    	String indicatedHeadingDeg = "get /instrumentation/heading-indicator/indicated-heading-deg";
    	
        this.offset = offset;
        this.xCordinate = xStart;
        this.yCordinate = yStart;
            new Thread(() -> {
                Socket socket = null;
                try {
                    ...
                    while(!stop){
                        ...
                        String[] x = longtitude.split(splitSymbol);
                        String[] y = latitude.split(splitSymbol);
                    
                        br.readLine();
                        out.println(indicatedHeadingDeg);
                        out.flush();
                        
                        String[] h = br.readLine().split(" ");
                        int tmp = h[3].length();
                        currentXLocation = Double.parseDouble(x[2]);
                        currentYLocation = Double.parseDouble(y[2]);
    ...
    private void routeStart() {
        ...
        pathX = xCordinate + (yPlane - 1) * offset;
        pathY = yCordinate - (xPlane - 1) * offset;
        endX = xCordinate + (yMark - 1) * offset;
        endY = yCordinate - (xMark - 1) * offset;
        ...
        while((!turn) && indexPlan<intersections.size()) {
            ...
            while ((Math.abs(currentXLocation - intersectionX)) > (radiusX * offset) || (Math.abs(currentYLocation - intersectionY)) > (radiusX * offset)) {
                try {
                    Thread.sleep(250);
                } catch (InterruptedException e) { e.printStackTrace(); }
            }
